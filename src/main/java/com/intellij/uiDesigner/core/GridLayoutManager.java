...
public final class GridLayoutManager
  ...
  public GridLayoutManager(int rowCount, int columnCount, Insets margin, int hGap, int vGap,
      boolean sameSizeHorizontally, boolean sameSizeVertically) {
  ...
  private Dimension getTotalGap(Container container, DimensionInfo hInfo, DimensionInfo vInfo) {
    ...
    return new Dimension(insets.left + insets.right +
        countGap(hInfo, 0, hInfo.getCellCount()) + this.myMargin.left + this.myMargin.right,
  ...
  private static int getDesignTimeInsets(Container container) {
    while (container != null) {
      if ((container instanceof JComponent)) {
        Integer designTimeInsets = (Integer) ((JComponent) container)
  ...
  public void layoutContainer(Container container) {
    ...
    for (int i = 0; i < layoutState.getComponentCount(); i++) {
      ...
      int cellWidth = this.myXs[(column + colSpan - 1)] + this.myWidths[(column + colSpan - 1)]
          - this.myXs[column];
      int cellHeight =
      ...
      component.setBounds(this.myXs[column] + dx, this.myYs[row] + dy, componentSize.width,
  ...
  private static void updateSizesFromChildren(DimensionInfo info, boolean min, int[] widths) {
    for (int i = info.getComponentCount() - 1; i >= 0; i--) {
      ...
      if ((c.isUseParentLayout()) && ((child instanceof Container))) {
        ...
        } else if ((container.getComponentCount() == 1) && ((container
  ...
  private static void updateSizesFromChild(DimensionInfo info, boolean min, int[] widths,
      Container container, int childIndex) {
    ...
    if (info.getSpan(childIndex) == info.getChildLayoutCellCount(childLayout)) {
      ...
      DimensionInfo childInfo = (info instanceof HorizontalInfo) ? childLayout.myHorizontalInfo
  ...
  private void new_doIt(int[] widths, int cell, int span, int minWidth, DimensionInfo info,
      boolean checkPrefs) {
  ...
  private static void distribute(boolean[] higherPriorityCells, DimensionInfo info,
      int toDistribute, int[] widths) {
    int stretches = 0;
  ...
  private void getCellsWithHigherPriorities(DimensionInfo info, boolean[] allowedCells,
      boolean[] higherPriorityCells, boolean checkPrefs, int[] widths) {
  ...
  public int[] getHorizontalGridLines() {
    ...
    result[this.myYs.length] = (this.myYs[(this.myYs.length - 1)] + this.myHeights[(this.myYs.length
        - 1)]);
    return result;
  }
  ...
  public int[] getVerticalGridLines() {
    ...
    result[this.myXs.length] = (this.myXs[(this.myXs.length - 1)] + this.myWidths[(this.myXs.length
        - 1)]);
    return result;
  }

...
public abstract class DimensionInfo {
  ...
  private int calcCellSizePolicy(int cellIndex) {
    ...
    return (canShrink ? 1 : 0) | ((canGrow) || ((countOfBelongingComponents > 0) && (weakCanGrow))
        ? 2 : 0) | ((wantGrow) || ((countOfBelongingComponents > 0) && (weakWantGrow)) ? 4 : 0);
  }
  ...
  public static Container findAlignedChild(Component child, GridConstraints c) {
    if ((c.isUseParentLayout()) && ((child instanceof Container))) {
      ...
      if ((container.getComponentCount() == 1) && ((container
  ...
  protected final Dimension getPreferredSize(int componentIndex) {
    ...
    if (size == null) {
      size = Util.getPreferredSize(this.myLayoutState.getComponent(componentIndex),
          this.myLayoutState.getConstraints(componentIndex), true);
  ...
  protected final Dimension getMinimumSize(int componentIndex) {
    ...
    if (size == null) {
      size = Util.getMinimumSize(this.myLayoutState.getComponent(componentIndex),
          this.myLayoutState.getConstraints(componentIndex), true);
